...
private fun getTriangleById(
    ...
    userStorage: UserStorage,
    jwtTools: JwtTools,
) = "/{triangle-id}".bind(Method.GET) to
    withErrorHandling {
        var userId = jwtTools.getExtractedUserIdAndValidate(it, userStorage)
        if (userId != null) {
            ...
            try {
                ...
                JsonUtils.getBasicErrorJsonResponse(
                    "Некорректное значение переданного параметра triangle-id. " +
...
private fun deleteTriangle(
    templateStorage: TemplateStorage,
    triangleStorage: TriangleStorage,
    userStorage: UserStorage,
    jwtTools: JwtTools,
) = "/{triangle-id}".bind(Method.DELETE) to
    withErrorHandling {
        var userId = jwtTools.getExtractedUserIdAndValidate(it, userStorage)
        if (userId != null) {
            val triangleIdString = it.path("triangle-id")
...
private fun getTrianglesSortedByBorderColor(
    templateStorage: TemplateStorage,
    triangleStorage: TriangleStorage,
    userStorage: UserStorage,
    jwtTools: JwtTools,
) = "/by-border-color".bind(Method.GET) to
    withErrorHandling {
        ...
        try {
            ...
            for (triangle in triangles) {
                ...
                if (template == null) {
                    ...
                    triangleByBorderColorDatas.add(
                        TriangleByBorderColorData(
                            triangle.id,
                            template.sideA,
                            template.sideB,
                            template.sideC,
            ...
            JsonUtils.getBasicErrorJsonResponse(
                "Некорректное значение переданного параметра border-color. " +
                    "Ожидается Color, но получено текстовое значение",
            )
        }
    }

private fun getTrianglesSortedByBorderArea(
    templateStorage: TemplateStorage,
    triangleStorage: TriangleStorage,
    userStorage: UserStorage,
    jwtTools: JwtTools,
) = "/by-area".bind(Method.GET) to
    withErrorHandling {
        ...
        if (areaMinString == null || areaMaxString == null) {
            ...
            if (areaMin == null || areaMax == null) {
                if (areaMin == null) {
                    JsonUtils.getBasicErrorJsonResponse(
                        String.format(
                            "Некорректное значение нижней границы площади. " +
                                "Для параметра area-min ожидается число, " +
                                "но получено текстовое значение «%s»",
                            areaMinString,
                        ),
                    )
                } else {
                    JsonUtils.getBasicErrorJsonResponse(
                        String.format(
                            "Некорректное значение верхней границы площади. " +
...
private fun getTrianglesStatistics(
    templateStorage: TemplateStorage,
    triangleStorage: TriangleStorage,
    userStorage: UserStorage,
    jwtTools: JwtTools,
) = "/statistics".bind(Method.GET) to
    withErrorHandling {
                ...
                JsonUtils.getBasicErrorJsonResponse(
                    String.format(
                        "Некорректное значение типа статистики. " +
                            "Для параметра by ожидается значение типа статистики, но получено «%s»",
                        byString,
                    ),
                )
            }
